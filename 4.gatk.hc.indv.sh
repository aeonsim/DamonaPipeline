...
#for chr in "${TARGET[@]}" 
#do
echo "#!/bin/bash" > hc-${3}.sh
echo "#SBATCH --mail-type=FAIL --requeue" >> hc-${3}.sh
echo "echo \${HOSTNAME}" >> hc-${3}.sh
echo "TARGET=(chr1 chr2 chr3 chr4 chr5 chr6 chr7 chr8 chr9 chr10 chr11 chr12 chr13 chr14 chr15 chr16 chr17 chr18 chr19 chr20 chr21 chr22 chr23 chr24 chr25 chr26 chr27 chr28 chr29 chrX chrM)" >> hc-${3}.sh
echo "$JAVA -Xmx3g -jar $GATK32 -R ${REF} -T HaplotypeCaller -L \${TARGET[\$SLURM_ARRAY_TASK_ID]} -I ${1} -o ${3}-\${TARGET[\$SLURM_ARRAY_TASK_ID]}-$VERSION.gatk.HC.refModel.vcf.gz -D ${DBSNP} -ped ${PED} --pedigreeValidationType SILENT --emitRefConfidence GVCF --variant_index_type LINEAR --variant_index_parameter 128000 --pair_hmm_implementation VECTOR_LOGLESS_CACHING" >> hc-${3}.sh
sbatch --ntasks=1 --cpus-per-task=1 --array=0-30 --mem-per-cpu=4000M --partition=${2}  hc-${3}.sh
#done

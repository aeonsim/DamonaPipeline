#!/bin/bash
#SBATCH --nodes=1 --ntasks-per-node=20 --mem-per-cpu=4000M
...
python ${PLATYPUS} callVariants --bamFiles=/tmp/${VERSION}.${SLURM_ARRAY_TASK_ID}.bams.list --regions=${TARGET[$SLURM_ARRAY_TASK_ID]} --output=${TARGET[$SLURM_ARRAY_TASK_ID]}-$VERSION.platypus.vcf --refFile=${REF} --assemble=1 --maxReads=15000000 --nCPU=$SLURM_JOB_CPUS_PER_NODE

#!/bin/bash
#SBATCH --nodes=1 --ntasks-per-node=2 --mem-per-cpu=5000M
...
$JAVA -Xmx8g -jar ${CRAM} merge -R ${REF} -r ${TARGET[$SLURM_ARRAY_TASK_ID]} $1/*cram |  $FREEBAYES -c -f ${REF} -r ${TARGET[$SLURM_ARRAY_TASK_ID]} | $BGZIP -c > /scratch/aeonsim/vcfs/${TARGET[$SLURM_ARRAY_TASK_ID]}-$VERSION.vcf.gz

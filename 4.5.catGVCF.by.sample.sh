#!/bin/bash
#SBATCH --nodes=1 --ntasks-per-node=1 --mem-per-cpu=2000M 
##--exclude=kosmos
...
NAMES=(`ls ${1}*refModel.vcf.gz | awk '{n=split($0,arra,"/"); print arra[n]}' | cut -f 1 -d "-" | cut -f 1 -d "." | uniq`)
...
$JAVA -Xmx1000m -cp $GATK32 org.broadinstitute.gatk.tools.CatVariants -R $REF -V ${NAMES[$SLURM_ARRAY_TASK_ID]}.list -out ${NAMES[$SLURM_ARRAY_TASK_ID]}.gvcf.vcf.gz
